...
@login_required
def glue(request, form_class=PhotoUploadForm,
        glue_model=Glueship, template_name="photos/glue.html", group_slug=None, bridge=None):
    """
    upload form for photos
    """
    ...
    if request.method == 'POST':
        if request.POST.get("action") == "upload":
            ...
            if photo_form.is_valid():
                ...
                request.user.message_set.create(message=_("Successfully uploaded photo '%s'") % photo.title)
                p = glue_model(branch_a=photo.id, type_a="2", branch_b=int(request.POST['ID']), type_b=request.POST['type'], created_by=request.user)
                p.save()
                include_kwargs = {"id": photo.id}
                if group:
                    redirect_to = bridge.reverse("photo_details", group, kwargs=include_kwargs)
...
def details(request, id, template_name="photos/details.html", group_slug=None, bridge=None, share_model = Share):
    ...
    if request.method == "POST":
        if request.POST["action"] == "share":
            if Share.objects.filter(shared_type=ContentType.objects.get(model='photo'), shared_id=photo.id).count() > 0:
                ...
                new_share = share_model(owner=request.user, shared=photo)
                new_share.save()
    return render_to_response(template_name, {
        "group": group,
        "host": host,
