...
def photo_window_info(photo):
    title = photo.title
    photo_url = photo.get_absolute_url()
    thumbnail_url = photo.get_thumbnail_url()
    info = '<div class="gallery-photo-thumb"><a href="'+ photo_url +'"><img src="'+ thumbnail_url + '" alt="' + title + '"/></a></div>';
    return info
register.simple_tag(photo_window_info)
