...
from account.models import OtherServiceInfo, RegisterCode
...
def signup(request, form_class=SignupForm,
    ...
    if request.method == "POST":
        form = form_class(request.POST)
        if form.is_valid():
            username, password, register_num = form.save()
            if settings.ACCOUNT_EMAIL_VERIFICATION:
                ...
                if register_num != 111:
                    register_code = RegisterCode.objects.get(register_number=register_num)
