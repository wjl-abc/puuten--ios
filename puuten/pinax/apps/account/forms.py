...
from account.models import Account, RegisterCode
...
class SignupForm(forms.Form):
    ...
    register_num = forms.IntegerField(label=_("Register Number"), widget=forms.TextInput())
...
#        raise forms.ValidationError(_("This username is already taken. Please choose another."))
    ...
    def clean_email(self):
        email_valid = User.objects.filter(email__iexact=self.cleaned_data["email"])
        if email_valid.count() != 0:
            raise forms.ValidationError(_("This email address is already taken. Please choose another."))
        return self.cleaned_data["email"]
    
    def clean_password1(self):
        password1 = self.cleaned_data["password1"]
        if len(password1)<7:
            raise forms.ValidationError(_("Your password should be no less than 7 characters"))
        return password1
    ...
    def clean_register_num(self):
        register_valid = RegisterCode.objects.filter(register_number=self.cleaned_data["register_num"], flag=0)
        if register_valid.count()<1 and self.cleaned_data["register_num"]!= 111:
            raise forms.ValidationError(_("Invalid register number. Please choose another."))
        return self.cleaned_data["register_num"]
    ...
    def save(self):
        ...
        register_num = self.cleaned_data["register_num"]
...
#        return username, password, register_num# required for authenticate()
        return username, password, register_num
