...
class UserSocialAuth(models.Model):
    ...
    def expires(self):
        ...
        the form of a float data type. None is returned if there's no
        value stored or it's malformed.
        """
        if self.extra_data:
            name = setting('SOCIAL_AUTH_EXPIRATION', 'expires')
            try:
                tmpextra_data = json.read(self.extra_data)
                return tmpextra_data[name]
    ...
    def expiration_delta(self):
        ...
        if self.extra_data:
            ...
            try:
                tmpextra_data = json.read(self.extra_data)
                return timedelta(seconds=int(tmpextra_data.get(name)))
            except (ValueError, TypeError):
                pass
        return None
