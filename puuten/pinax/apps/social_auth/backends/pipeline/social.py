# by jipan from django.db.utils import IntegrityError
...
def social_auth_user(backend, uid, user=None, *args, **kwargs):
    ...
    """
    try:
        social_user = UserSocialAuth.objects.select_related('user').get(provider=backend.name, uid=str(uid))
    except :# by jipan UserSocialAuth.DoesNotExist:
        social_user = None
...
def associate_user(backend, user, uid, social_user=None, *args, **kwargs):
    ...
    try:
        social = UserSocialAuth.objects.create(user=user, uid=str(uid), provider=backend.name)
    except :#by jipan IntegrityError:
...
def load_extra_data(backend, details, response, social_user, uid, user,
    ...
    if setting('SOCIAL_AUTH_EXTRA_DATA', True):
        ...
        if extra_data and social_user.extra_data != extra_data:
            social_user.extra_data = json.dumps(extra_data,cls=JSONEncoder)
